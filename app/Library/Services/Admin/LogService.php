...
class LogService extends BaseService
{
    ...
    {
        $data = LoginHistory::with('user')->select('*');
        ...
        return Datatables::of($data)
            ...
            ->editColumn('user_id', function ($row) {
                return $row->user_id ? $row->user->email : 'N/A';
            })
            ->editColumn('created_at', function ($row) {
                return $row->created_at ? getFormattedDateTime($row->created_at) : 'N/A';
            })
            ->addColumn('status', function ($row) {
                return $row->status == "success" ? '<div class="text-capitalize text-primary">' . $row->status . '</div>' : '<div class="text-capitalize text-danger">' . $row->status . '</div>';
            })
            ->addColumn('action', function ($row) use ($user_role) {
                $actionHtml = '';

                if (Helper::hasAuthRolePermission('log_delete_login')) {
                    $actionHtml = '<a class="dropdown-item text-primary" href="javascript:void(0)" onclick="confirmModal(deleteLoginHistory, ' . $row->id . ', \'Are you sure you want to delete this record?\')" ><i class="far fa-trash-alt"></i> Delete</a>';
                }
        ...
        return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) use ($user_role) {
                    ...
                    if (Helper::hasAuthRolePermission('log_activity_delete')) {
                        $actionHtml = '<a class="dropdown-item text-primary" href="javascript:void(0)" onclick="confirmModal(deleteActivityLog, ' . $row->id . ', \'Are you sure you want to delete this record?\')" ><i class="far fa-trash-alt"></i> Delete</a>';
                    }
        ...
        return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) use ($user_role) {
                    ...
                    if (Helper::hasAuthRolePermission('log_email_delete')) {
                        $actionHtml .= '<a class="dropdown-item text-primary" href="javascript:void(0)" onclick="confirmModal(deleteEmailHistory, ' . $row->id . ', \'Are you sure you want to delete this record?\')" ><i class="far fa-trash-alt"></i> Delete</a>';
                    }
