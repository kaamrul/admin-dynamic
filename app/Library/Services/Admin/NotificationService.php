...
class NotificationService extends BaseService
{
    ...
    private function actionHtml($row, $user_role)
    {
        ...
        if (Helper::hasAuthRolePermission('notification_delete')) {
            $actionHtml .= '<a class="dropdown-item text-danger" href="javascript:void(0)" onclick="confirmModal(deleteNotification, ' . $row->id . ', \'Are you sure you want to delete this record?\')" ><i class="far fa-trash-alt"></i> Delete </a>';
        }
    ...
    public function dataTable(array $filter_params)
    {
        ...
        return Datatables::of($data)
            ...
            ->editColumn('subject', function ($row) {
                return '<a class="text-primary" href="javascript:void(0)" onclick="showViewModal(\'' . htmlspecialchars(json_encode($row->message), ENT_QUOTES, 'UTF-8') . '\', \'' . $row->subject . '\', \'' . $row->send_date . '\')" > ' . Str::limit($row->subject, 40) . ' </a>';
            })
    ...
    public function createNotification(array $data): bool
    {
        try {
            ...
            if ($data['member_type'] == 2) {
                foreach ($data['tournament'] as $id) {
                    $tournament = Tournament::find($id);

                    foreach ($tournament->teams as $team) {
                        foreach ($team->teamAnglers->load('member.user') as $angler) {
                            $notification_recipient['user_id'] = $angler->member->user->id;
