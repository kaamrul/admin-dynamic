...
class RoleController extends Controller
{
    ...
    private function getActionHtml($row)
    {
        ...
        if ($row->slug != Enum::USER_TYPE_SUPER_ADMIN) {
            ...
            if (Helper::hasAuthRolePermission('role_delete')) {
                $actionHtml .= '<a class="dropdown-item text-danger" href="javascript:void(0)" onclick="confirmModal(deleteRole, ' . $row->id . ', \'Are you sure you want to delete this record?\')" ><i class="far fa-trash-alt"></i> Delete</a>';
            }
        } else {
            return '';
        }
    ...
    public function permissions(Role $role)
    {
        ...
        $all_permissions = Permission::get()->groupBy('menu_group')->map(function ($group) {
