...
class RoleMiddleware
{
    ...
    public function handle(Request $request, Closure $next, $role)
    {
        ...
        if ($guard->check() &&
             ($guard->user()->isEmployee() ||
             $guard->user()->isSuperAdmin()) &&
             $guard->user()->status != Enum::USER_STATUS_ACTIVE) {
    ...
    private function userHasPermission($guard, $role)
    {
        if ($guard->check() && (
            ...
            ($guard->user()->user_type === Enum::USER_TYPE_SUPER_ADMIN && $role == 'admin') ||
            ($guard->user()->user_type === Enum::USER_TYPE_EMPLOYEE && $role == 'admin')
        )) {
            return true;
        }
